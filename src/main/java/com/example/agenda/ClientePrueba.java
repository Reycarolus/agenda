...
public class ClientePrueba {
    ...
    public void realizarPruebas(Contacto nuevoContacto) {
        ...
        // Le cambiamos el teléfono al objeto Java
        nuevoContacto.setTelefono("666666666");
        // Enviamos el cambio al servidor
        Contacto contactoModificado = modificarContacto(nuevoContacto.getId(), nuevoContacto);
        System.out.println("Contacto modificado: " + contactoModificado);
        // Listamos otra vez para confirmar que el cambio se ha guardado en el servidor
        listarContactos();
    ...
    private Contacto modificarContacto(Long id, Contacto contacto) {
        ...
        //Preparamos las cabeceras para decir que enviamos JSON
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);

        //Empaquetamos el contacto y las cabeceras en una "Entidad HTTP"
        HttpEntity<Contacto> requestEntity = new HttpEntity<>(contacto, headers);

        //Enviamos la petición con el metodo PUT (HttpMethod.PUT)
        //la URL es BASE_URL + "/" + id
